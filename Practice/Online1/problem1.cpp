...
public:
    ...
    static double custom(const vector<vector<int>> &grid)
    {
        ...
        // top right
        cornerConflict += (grid[0][k - 1] == k - 1 && grid[0][k - 2] == 2 * k - 1 && grid[1][k - 1] == k - 2) ? 1 : 0;

        // bottom-left
        cornerConflict += (grid[k - 1][0] == k * (k - 1) && grid[k - 1][1] == k * (k - 2) && grid[k - 2][0] == k * (k - 1) + 1) ? 1 : 0;

        // bottom-right
        cornerConflict += (grid[k - 1][k - 1] == k * k - 1 && grid[k - 1][k - 2] == (k - 2) * k + k - 1 && grid[k - 2][k - 1] == k * k - 1) ? 1 : 0;
...
bool isSolvable(const vector<vector<int>> &grid)
{
    ...
    if (k % 2 == 0 && (blankRowBottom + inversions) % 2 == (k + 0) % 2)
        return true;
    return false;
}
